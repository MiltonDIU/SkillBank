        ...
        <div class="card-body">
            <form method="POST" action="{{ route("admin.articles.update", [$article->id]) }}" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('slug') ? 'is-invalid' : '' }}" type="text" name="slug" id="slug" value="{{ old('slug', $article->slug) }}" required>
                    @if($errors->has('slug'))
                        <div class="invalid-feedback">
                            {{ $errors->first('slug') }}
                        </div>
                ...
                <div class="form-group">
                    ...
                    <textarea class="form-control {{ $errors->has('summary') ? 'is-invalid' : '' }}" name="summary" id="summary">{{ old('summary', $article->summary) }}</textarea>
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('icon_class_name') ? 'is-invalid' : '' }}" type="text" name="icon_class_name" id="icon_class_name" value="{!! old('content', $article->icon_class_name) !!}">
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('menu') ? 'is-invalid' : '' }}" name="menu_id" id="menu_id">
                        @foreach($menus as $id => $entry)
                            <option value="{{ $id }}" {{ (old('menu_id') ? old('menu_id') : $article->menu->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                        @endforeach
                    </select>
                    @if($errors->has('menu'))
                        <div class="invalid-feedback">
                ...
                <div class="form-group">
                    ...
                    <select class="form-control {{ $errors->has('is_status') ? 'is-invalid' : '' }}" name="is_status" id="is_status" required>
                        ...
                        @foreach(App\Models\Article::IS_STATUS_SELECT as $key => $label)
                            <option value="{{ $key }}" {{ old('is_status', $article->is_status) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
